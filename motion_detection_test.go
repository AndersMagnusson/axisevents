...
func TestMotionDetectionRecordingOnSdCardLongRunning(t *testing.T) {
	...
	err := NewMotionDetectionHandler("newtest", true, 2).Record().Video().SdCard().ExecuteOn(context.TODO(), device)
...
func TestHttpNotificationLongRunning(t *testing.T) {
	...
	err := NewMotionDetectionHandler("httpNotifications", true, 2).HttpNotification().Notify("testing", "source=livingroom").HTTP("http://192.168.1.106", "", "", "", "", "", "", "").ExecuteOn(context.TODO(), device)
